...
function showClassifiList(result) {
    ...
    for(let i = 0; i < result.length; i++) {
        let html = "";
        if(result[i].classifi == classification) {
            html += `<div class="form-check"><input type="radio" class="form-check-input" name="classifi" value="${result[i].classifi}" checked>${result[i].k_name}</div>`;
        }else {
            html += `<div class="form-check"><input type="radio" class="form-check-input" name="classifi" value="${result[i].classifi}">${result[i].k_name}</div>`;
        }
...
document.addEventListener('click', (e)=> {
    ...
    if(selectSearchedData.classList[0] == 'autocomp') {
        document.getAutocompletePnr_name = selectSearchedData.innerText;
        document.querySelector('.pop_keywords').innerHTML = '';
        getAutocompletePnr_name(selectSearchedData).then(result => {
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        if(result[i].classifi == classification) {
            if(result[i].vendor == supplier) {
                html += `<div class="form-check"><input type="radio" class="form-check-input" name="vendor" value="${result[i].vendor}" checked>${result[i].vendor_name}</div>`;
            }else {
                html += `<div class="form-check"><input type="radio" class="form-check-input" name="vendor" value="${result[i].vendor}">${result[i].vendor_name}</div>`;
            }
        }else {
            html += `<div class="form-check"><input type="radio" class="form-check-input" name="vendor" value="${result[i].vendor}">${result[i].vendor_name}</div>`;
        }
